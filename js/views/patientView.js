  ...
  const html = `
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <form id="book-appointment-form" class="space-y-5">
                    <div>
                        ...
                        <select id="select-doctor" class="block w-full rounded-md border-gray-300 shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm px-3 py-2 border" required>
                            ...
                            ${AppState.clinic.doctors.map((d) => `<option value="${d.id}">Dr. ${d.name} (${d.specialty})</option>`).join("")}
                        </select>
                    </div>
                    <div>
  ...
  document
    ...
    .addEventListener("submit", (e) => {
      ...
      try {
        ...
        renderView("Patient"); // Re-render to show updated appointments
      } catch (error) {
        console.error(error);
